...
int SDFileSystem::clk_frequency()
{
	return m_FREQ;
}
...
printf("temp log The card is now initialized \n");
...
printf("temp log The card initialize done \n");
...
int SDFileSystem::disk_send_status(uint32_t wait_ms)
{
	int timeout = 1;
	char ret = 0;
	unsigned int resp = 0;
//	DigitalIn busy(PA_6);
	...
	m_Timer.start();
	do {
		ret = commandTransaction(CMD13, 0x00000000, &resp);

		if (ret == 0x00 && resp == 0x00) {
			timeout = 0;
//			printf ("22busy : %d\n", busy);
			break;
		}
...
int SDFileSystem::disk_erase(uint32_t sector, uint32_t count)
{
	int ret = 0;
    //Make sure the card is initialized before proceeding
    if (m_Status & STA_NOINIT)
	...
	ret = writeCommand(CMD33, sector + count - 1);
	...
	ret = disk_send_status(5000);

	if (ret) {
		// Busy timeout after erase
		error("[%s][%d] Card not released busy while %d ms\n", __func__, __LINE__, 5000);
	}
...
bool SDFileSystem::readCid()
{
    ...
    for (int f = 0; f < 3; f++) {
        ...
        //Send CMD10(0x00000000) to read the CID register
        if (writeCommand(CMD10, 0x00000000) == 0x00) {
            //Read the 16B CID data block
            //char cid[16];
			int cid[4];
            bool success = readData((char*)cid, 16);
            deselect();
            ...
            if (success) { 
				...
				m_cid.manfid        = UNSTUFF_BITS(cid, 120, 8);
				m_cid.oemid         = UNSTUFF_BITS(cid, 104, 16);
				m_cid.prod_name[0]      = UNSTUFF_BITS(cid, 96, 8);
				m_cid.prod_name[1]      = UNSTUFF_BITS(cid, 88, 8);
				m_cid.prod_name[2]      = UNSTUFF_BITS(cid, 80, 8);
				m_cid.prod_name[3]      = UNSTUFF_BITS(cid, 72, 8);
				m_cid.prod_name[4]      = UNSTUFF_BITS(cid, 64, 8);
				m_cid.hwrev         = UNSTUFF_BITS(cid, 60, 4);
				m_cid.fwrev         = UNSTUFF_BITS(cid, 56, 4);
				m_cid.serial        = UNSTUFF_BITS(cid, 24, 32);
				m_cid.year          = UNSTUFF_BITS(cid, 12, 8);
				m_cid.month         = UNSTUFF_BITS(cid, 8, 4);
...
bool SDFileSystem::readData(char* buffer, int length)
{
    ...
    ret = (!m_Crc || crc == SDCRC::crc16(buffer, length));
	//printf ("[%s][%d] ret : %d crc : 0x%x, crc2 : 0x%x\n", __func__, __LINE__, ret, crc, SDCRC::crc16(buffer, length));
...
inline bool SDFileSystem::writeBlock(const char* buffer, unsigned int lba)
{
    ...
    for (int f = 0; f < 3; f++) {
printf("temp log writblock 12\r\n");
        //Select the card, and wait for ready
        if(!select())
            break;
printf("temp log writblock 13\r\n");
...
struct SDFileSystem::mmc_cid* SDFileSystem::get_cid()
{
	return &m_cid;
}