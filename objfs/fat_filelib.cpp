...
#else
	return fatfs_format_fat32(&_fs, volume_sectors, name);
#endif
}
...
// RW 공유 Buffer를 이용하지 않고 바로바로 미디어에 write 함
// 기존의 write는 sector size 단위의 버퍼를 만들어 놓고 해당 버퍼가 가득 차면 
// 미디어에 write 하는 방식을 채택하고 있음. 
...
// RW 공유 Buffer를 이용하지 않고 바로바로 미디어에서 read 함
// 기존의 read는 sector size 단위의 버퍼를 만들어 놓고 해당 버퍼의 내용이 
// 사용자가 원하는 내용일 경우, 버퍼를 리턴함. (Buffer는 write와 read가 공유함)
...
// 바이트 단위로 RW를 하도록 하기위해 함수 등록
// rd : 바이트 단위 read 함수가 등록되어야 함
// wr : 바이트 단위 write 함수가 등록되어야 함
