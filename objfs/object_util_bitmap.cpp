...
static offset_t get_free_offset(char *p_map, const uint32 size_b_map, uint32 data_length)
{
	...
	// TBD : bit 연산을 통한 free area 찾는 알고리즘을 더 효율적으로 변경할 수 있을 것 같다...
...
// 버그 존재
// 역할을 아래와 같다. 
// 4096byte 이상의 free 영역을 리턴
// bitmap 의 free offset을 리턴을 최적화 
offset_t get_free_offset_extend(struct OBJECT *p_object, uint32 data_length)
{
	offset_t save_offset = 0;
	offset_t offset = 0;
	offset_t sli_offset = 0;			//필요 조건에 부합하지 않는, 마지막 bitmap에서 구해진 offset, 
