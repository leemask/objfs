...
#include "plr_system_mbed.h" //andrew
//-----------------------------------------------------------------------------
...
printf("First disk io \n");
    // Load MBR (LBA 0) into the 512 byte buffer
    if (!fs->disk_io.read_media(0, fs->currentsector.sector, 1))
        return FAT_INIT_MEDIA_ACCESS_ERROR;
printf("First disk io done %x %x\r\n",fs->currentsector.sector[0], fs->currentsector.sector[SIGNATURE_POSITION]);
    ...
    // Else possibly MBR less disk
        ...
        fs->lba_begin = 0;
printf("temp log lba_begin %d, code 0x%x, location 0x%x \r\n",  fs->lba_begin, fs->currentsector.sector[PARTITION1_TYPECODE_LOCATION], PARTITION1_LBA_BEGIN_LOCATION);
...
int fatfs_sector_read_byte(struct fatfs *fs, uint32 addr, uint8 *target, uint32 length_b)
{
    return fs->disk_io.read_byte_media(addr, target, length_b);
}

int fatfs_sector_write_byte(struct fatfs *fs, uint32 addr, uint8 *target, uint32 length_b)
{
	return fs->disk_io.write_byte_media(addr, target, length_b);
}

// 디렉터리 Entry에서 파일의 시작 클러스트 주소를 업데이트한다.
int fatfs_update_start_cluster(struct fatfs *fs, uint32 Cluster, char *shortname, uint32 start_cluster)
{
    uint8 item=0;
    uint16 recordoffset = 0;
    int x=0;
        ...
        if (fatfs_sector_reader(fs, Cluster, x++, 0)) // If sector read was successfull
        {
            ...
            for (item = 0; item < FAT_DIR_ENTRIES_PER_SECTOR; item++)
            {
                ...
                if (fatfs_entry_sfn_only(directoryEntry) )
                {
                    if (strncmp((const char*)directoryEntry->Name, shortname, 11)==0)
                    {
						directoryEntry->FstClusHI = start_cluster >> 16 & 0x00FF;
						directoryEntry->FstClusLO = (start_cluster << 16) >> 16 & 0x00FF;
#if FATFS_INC_TIME_DATE_SUPPORT
                        // Update access / modify time & date
                        fatfs_update_timestamps(directoryEntry, 0, 1, 1);
#endif
