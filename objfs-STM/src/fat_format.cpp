...
static int fatfs_create_boot_sector(struct fatfs *fs, uint32 boot_sector_lba, uint32 vol_sectors, const char *name, int is_fat32)
{
    ...
    else
    {
		...
		//fs->currentsector.sector[90] = ((fs->object_first_cluster) >> 0) & 0xFF;
		//fs->currentsector.sector[91] = ((fs->object_first_cluster) >> 8) & 0xFF;
		//fs->currentsector.sector[92] = ((fs->object_first_cluster) >> 16) & 0xFF;
		//fs->currentsector.sector[93] = ((fs->object_first_cluster) >> 24) & 0xFF;
		//-------------------------------------------------------------------------
...
static int fatfs_erase_fat(struct fatfs *fs, int is_fat32)
{
    ...
    else
    {
		...
		//ieroa@elixirflash.com-------------------------------------
		//for saving a Object Header Info
		//SET_32BIT_WORD(fs->currentsector.sector, 12, 0x0FFFFFFF);
		//----------------------------------------------------------
    }
