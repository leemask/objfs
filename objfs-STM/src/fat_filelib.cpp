...
// Locals
//-----------------------------------------------------------------------------
static FL_FILE            _files[FATFS_MAX_OPEN_FILES+2];
...
void* fl_fopen(const char *path, const char *mode)
{
    ...
    FL_FILE* file;
    uint8 flags = 0;
	printf("filelib_valid = %d\r\n", _filelib_valid);
    // If first call to library, initialise
	printf("fl_fopen....\r\n");
    CHECK_FL_INIT();

    if (!_filelib_valid) {
		printf("filelib not valid\r\n");
        return NULL;
	}
...
#ifndef OBJECT_FS
	ret = fatfs_format(&_fs, volume_sectors, name);
#else
	ret = fatfs_format_fat32(&_fs, volume_sectors, name);
#endif
	printf("valid %d \r\n", _filelib_valid);
	return ret;

}
int get_filelib_valid()
{
	return _filelib_valid;
}
...
// ieroa@elixirflash.com---------------------------------------------------
uint32 fl_get_file_length(void *f)
{
	FL_FILE *file = (FL_FILE *)f;
...
// RW ??À¯ Buffer?? ?Ì¿????? ?Ê°? ?Ù·Î¹Ù·? ?Ìµ??î¿¡ write ??
// ??Á¸?? write?? sector size ??À§?? ???Û¸? ?????? ???? ?Ø´? ???Û°? ???? ???? 
// ?Ìµ??î¿¡ write ?Ï´? ????À» Ã¤???Ï°? ??À½. 
int fl_fdirect_write(const void * data, int size, int count, void *f )
{
    FL_FILE *file = (FL_FILE *)f;
    uint32 offset;
    uint32 length = (size*count);
	...
	uint32 i = 0;
	uint32 LastCluster = FAT32_LAST_CLUSTER;
	const uint32 TotalWriteCount = 1;
	uint32 addr = 0;
    ...
    offset = file->bytenum % (FAT_SECTOR_SIZE * _fs.sectors_per_cluster);
	ClusterIdx = (file->bytenum / FAT_SECTOR_SIZE) / _fs.sectors_per_cluster;
	...
	addr = (fatfs_lba_of_cluster(&_fs, Cluster) * FAT_SECTOR_SIZE) + offset;
	fatfs_sector_write_byte(&_fs, addr, (uint8*)data, size);

	file->bytenum  = file->bytenum + length;
...
// RW ??À¯ Buffer?? ?Ì¿????? ?Ê°? ?Ù·Î¹Ù·? ?Ìµ??î¿¡?? read ??
// ??Á¸?? read?? sector size ??À§?? ???Û¸? ?????? ???? ?Ø´? ?????? ?????? 
// ?????Ú°? ???Ï´? ?????? ????, ???Û¸? ??????. (Buffer?? write?? read?? ??À¯??)
int fl_fdirect_read(void * buffer, int size, int count, void *f )
{
    uint32 offset = 0;
	uint32 ClusterIdx = 0;
	uint32 Cluster = 0;
    ...
    if (buffer==NULL || file==NULL)
        return -1;
	printf("fl_fdirect_read 1 \r\n");

    // No read permissions
    if (!(file->flags & FILE_READ))
        return -1;
	printf("fl_fdirect_read 2 \r\n");
    ...
    if (file->bytenum >= file->filelength)
        return -1;
	printf("fl_fdirect_read 3 \r\n");

    // Limit to file size
    if ( (file->bytenum + length) > file->filelength )
        length = file->filelength - file->bytenum;
	...
	addr = (fatfs_lba_of_cluster(&_fs, Cluster) * FAT_SECTOR_SIZE) + offset;
	if(fatfs_sector_read_byte(&_fs, addr, (uint8*)buffer, length) < 0)
		return 0;
...
// rd : ????Æ® ??À§ read ?Ô¼??? ???ÏµÇ¾??? ??
// wr : ????Æ® ??À§ write ?Ô¼??? ???ÏµÇ¾??? ??
int fl_attach_media_byte_io(fn_diskio_read_byte rd, fn_diskio_write_byte wr)
{
    int res;
	...
	_fs.disk_io.read_byte_media = rd;
	_fs.disk_io.write_byte_media = wr;
...
// FAT???? ??Á¤?? Å¬????Æ® ?????î¸¦ ????
uint32 fl_get_cluster_size(void)
{
	return _fs.sectors_per_cluster * FAT_SECTOR_SIZE;
}

// ?????? ???? Å¬?????Íº??? count ??Å­ invalid Ã³?? ?Ñ´?. 
int fl_invalid_cluster(void *f, uint32 count)
{
	FL_FILE *p_file = (FL_FILE*)f;
	uint32 invalid_cluster = 0;
	uint32 next_cluster = 0;
	uint32 i = 0;

	invalid_cluster = p_file->startcluster;

	FL_LOCK(&_fs);

	for(i = 0; i < count; i++)
	{
		next_cluster = fatfs_find_next_cluster(&_fs, invalid_cluster);
		if(!fatfs_fat_set_cluster(&_fs, invalid_cluster, FAT32_INVALID_CLUSTER))
			return -1;

		invalid_cluster = next_cluster;
	}

	if(!fatfs_update_start_cluster(&_fs, p_file->parentcluster, (char*)p_file->shortfilename, next_cluster))
		return -1;

	p_file->startcluster = next_cluster;
	p_file->filelength -= ((FAT_SECTOR_SIZE * _fs.sectors_per_cluster) * count);
	p_file->bytenum = p_file->filelength;
	p_file->last_fat_lookup.CurrentCluster = next_cluster;
	FL_UNLOCK(&_fs);
	return 0;
}
//-------------------------------------------------------------------------
